...
const SaleCards = () => {
  ...
  const fetchAndFilterDiscountedProducts = async (
    minPrice,
    maxPrice,
    sortOption
  ) => {
    ...
    try {
      ...
      const fetchedProducts = await fetchMultipleDiscountedPages(
        maxPages,
        100,
        sortOption
      );
      ...
      if (fetchedProducts && fetchedProducts.length > 0) {
        ...
        if (filteredProducts.length > 0) {
          ...
          setFilterError(
            "No discounted products found in the specified price range"
          );
        }
      } else {
        setFilterError(
  ...
  const fetchMoreFilteredProducts = async () => {
    ...
    try {
      ...
      const additionalProducts = await fetchMultipleDiscountedPages(
        pagesToFetch,
        100,
        sortOption,
        startPage
