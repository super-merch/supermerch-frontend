...
const SaleCards = () => {
  ...
  const {
    ...
    fetchMultipleDiscountedPages, // This needs to be added to AppContext
    discountedProducts: contextDiscountedProducts,
    skeletonLoading,
    marginApi,
    totalDiscount,
    backendUrl,
  ...
  useEffect(() => {
    if (contextDiscountedProducts) {
      dispatch(setProducts(contextDiscountedProducts));
    }
  }, [contextDiscountedProducts, dispatch]);
  ...
  // Function to fetch and filter discounted products with price range
  const fetchAndFilterDiscountedProducts = async (minPrice, maxPrice, sortOption) => {
    setIsFiltering(true);
    setFilterError("");
    ...
    try {
      ...
      // Use the fetchMultipleDiscountedPages method from AppContext
      const fetchedProducts = await fetchMultipleDiscountedPages(maxPages, 100, sortOption);
      ...
      if (fetchedProducts && fetchedProducts.length > 0) {
        ...
        if (filteredProducts.length > 0) {
          ...
          setFilterError("No discounted products found in the specified price range");
        }
      } else {
        setFilterError("No discounted products found in the specified price range");
      }
  ...
  const fetchMoreFilteredProducts = async () => {
    ...
    try {
      ...
      const additionalProducts = await fetchMultipleDiscountedPages(pagesToFetch, 100, sortOption, startPage);
      ...
      console.error("Error fetching more discounted products:", error);
    } finally {
      setIsFiltering(false);
    }
  };
  ...
  useEffect(() => {
    if (isPriceFilterActive) {
      fetchAndFilterDiscountedProducts(minPrice, maxPrice, sortOption);
    } else {
  ...
  const getCurrentPageProducts = () => {
    if (isPriceFilterActive) {
      ...
      // Use regular discounted products with local search filter
          ...
          <div className="lg:hidden px-4 py-3">
            ...
            <div className="mb-6 px-2">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-brand text-base">{!skeletonLoading && !isFiltering && getTotalCount()}</span>
                <p className="text-sm text-gray-600">
                  {skeletonLoading || isFiltering
                    ? "Loading..."
                    : `Results found (Sale Products)${isPriceFilterActive ? " (Price filtered)" : ""}`}
          ...
          >
            {skeletonLoading || isFiltering ? (
                  ...
                  .map((product) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="max-h-[25%] sm:max-h-[71%] h-full border-b overflow-hidden relative">
                          <img
              ...
              <div className="flex items-center justify-center h-full">
                <p className="pt-10 text-xl text-center text-red-500">No Discounted Products Found</p>
              </div>
            )}
          </div>
