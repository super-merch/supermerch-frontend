...
const TrendCards = () => {
  ...
  const fetchAndFilterTrendingProducts = async (minPrice, maxPrice, sortOption) => {
    setIsFiltering(true);
    setFilterError("");
    ...
    try {
      ...
      const fetchedProducts = await fetchMultipleTrendingPages(maxPages, 100, sortOption);
  ...
  const fetchMoreFilteredProducts = async () => {
    ...
    try {
      ...
      const additionalProducts = await fetchMultipleTrendingPages(pagesToFetch, 100, sortOption, startPage);
  ...
  useEffect(() => {
    if (isPriceFilterActive) {
      fetchAndFilterTrendingProducts(minPrice, maxPrice, sortOption);
    } else {
  ...
  useEffect(() => {
    if (!isPriceFilterActive) {
      fetchTrendingProducts(currentPage, sortOption);
    }
  ...
  const getCurrentPageProducts = () => {
    if (isPriceFilterActive) {
      ...
      // Use regular trending products with local search filter
            ...
            <div className="flex items-center gap-1">
              ...
              <p className="">
                {isLoading || isFiltering
                  ? "Loading..."
                  : `Trending Results found${selectedCategory ? " (Category)" : " (All Products)"}${
          ...
          >
            {skeletonLoading || isFiltering ? (
              ...
              <div className="flex items-center justify-center h-full">
                <p className="pt-10 text-xl text-center text-red-500">No Trending Products Found</p>
              </div>
            )}
          </div>
