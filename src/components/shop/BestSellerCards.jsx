...
const BestSellerCards = () => {
  ...
  const fetchAndFilterBestSellerProducts = async (
    minPrice,
    maxPrice,
    sortOption
  ) => {
    ...
    try {
      ...
      const fetchedProducts = await fetchMultipleBestSellerPages(
        maxPages,
        100,
        sortOption
      );
      ...
      if (fetchedProducts && fetchedProducts.length > 0) {
        ...
        if (filteredProducts.length > 0) {
          ...
          setFilterError(
            "No best seller products found in the specified price range"
          );
        }
      } else {
        setFilterError(
  ...
  const fetchMoreFilteredProducts = async () => {
    ...
    try {
      ...
      const additionalProducts = await fetchMultipleBestSellerPages(
        pagesToFetch,
        100,
        sortOption,
        startPage
            ...
            <div className="flex items-center justify-between px-3 py-3 lg:w-[43%] md:w-[42%] w-full">
              {!isPriceFilterActive && (
                <>
                  <input
                    type="text"
                    placeholder="Search for best seller products..."
