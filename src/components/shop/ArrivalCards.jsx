...
const ArrivalCards = () => {
  ...
  const fetchAndFilterArrivalProducts = async (minPrice, maxPrice, sortOption) => {
    setIsFiltering(true);
    setFilterError("");
    ...
    try {
      ...
      const fetchedProducts = await fetchMultipleArrivalPages(maxPages, 100, sortOption);
  ...
  const fetchMoreFilteredProducts = async () => {
    ...
    try {
      ...
      const additionalProducts = await fetchMultipleArrivalPages(pagesToFetch, 100, sortOption, startPage);
          ...
          >
            {skeletonLoading || isFiltering ? (
              ...
              <div className="flex items-center justify-center h-full">
                <p className="pt-10 text-xl text-center text-red-500">No New Arrival Products Found</p>
              </div>
            )}
          </div>
