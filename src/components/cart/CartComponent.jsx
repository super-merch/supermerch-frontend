...
const CartComponent = () => {
  ...
  const totalDiscountPercent = items.reduce((sum, item) => sum + (totalDiscount[item.id] || 0), 0);

  // Base total calculation
  const totalAmount = items.reduce((sum, item) => sum + (item.totalPrice || item.price * item.quantity), 0);
  ...
  const handleFileChange = (e, productId) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        dispatch(updateCartItemImage({ id: productId, dragdrop: reader.result }));
      };
  ...
  const handleApplyCoupon = async () => {
    ...
    if (appliedCoupon) {
      toast.error("A coupon is already applied. Remove it first to apply a new one.");
      return;
    }
  ...
  return (
    <div className="Mycontainer !mb-10 mt-5">
      ...
      <div className="flex items-center justify-between mb-6 flex-col md:flex-row">
        <h1 className="text-2xl font-bold text-gray-900 mb-4 md:mb-0">Shopping Cart ({items?.length})</h1>
                              ...
                              <div className="flex items-center justify-center">
                                <div className="bg-white border border-gray-300 rounded-lg overflow-hidden shadow-sm">
                                  <div className="flex items-center">
                                    ...
                                    <input
                                      type="number"
                                      value={customQuantities[item.id] || item.quantity}
                                      onChange={(e) =>
                                        dispatch(
                                          multipleQuantity({
                                            id: item.id,
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="text-left">
                                <div className="text-sm font-bold text-smallHeader">${item.price.toFixed(2)}</div>
                                <div className="text-xs text-gray-500">per unit</div>
                              </div>
            ...
            <div className="md:w-1/3 ml-auto mt-6 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4">
                <div className="mb-3">
                  ...
                  <div className="flex justify-between items-center py-2">
                    <span className="text-sm font-medium text-gray-600">Tax (GST 10%)</span>
                    <span className="text-lg font-bold text-gray-900">${(gstAmount || 0).toFixed(2)}</span>
                  </div>

                  {appliedCoupon && (
                    <div className="flex justify-between items-center py-2">
                      <span className="text-sm font-medium text-green-600">Coupon Discount</span>
                      <span className="text-lg font-bold text-green-600">-${(couponDiscountAmount || 0).toFixed(2)}</span>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {appliedCoupon && (
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-green-600 font-medium">{appliedCoupon.coupen} Applied</span>
                        <button onClick={handleRemoveCoupon} className="text-red-500 hover:text-red-700 text-sm font-medium underline">
                          Remove
                        </button>
                      </div>
                    )}
      ...
      {openModel && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            {/* Backdrop */}
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onClick={() => setOpenModel(false)}></div>
              ...
              <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    <h3 className="text-lg font-semibold leading-6 text-gray-900">Remove Item</h3>
                    <div className="mt-2">
