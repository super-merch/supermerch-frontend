...
const SearchBar = ({
  ...
  const [suggestionResultType, setSuggestionResultType] = useState("none"); // "category" | "product" | "none"
        ...
        <div className="flex items-center bg-white border-2 border-blue-200 rounded-xl px-3 py-1.5 hover:border-blue-300 shadow-lg transition-all duration-300 w-full animate-in slide-in-from-right-2 fade-in-0">
          {showCategoryDropdown && (
            ...
            >
              ...
              <svg
                className={`w-4 h-4 text-primary transition-transform duration-200 ${isCategoryDropdownOpen ? "rotate-180" : ""
                  }`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
          ...
          {isSuggestionsOpen && suggestions.length > 0 && (
            ...
            >
              ...
              {suggestionResultType === "category" && (
                <li className="px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 border-b border-blue-100">
                  Categories
                </li>
              )}
              {suggestions.map((s, idx) => (
                <li
                  ...
                  className={`cursor-pointer px-3 py-2 text-sm flex justify-between items-center ${highlightedIndex === idx
                      ? "bg-gray-100"
                      : "hover:bg-gray-50"
                    }`}
                >
                  <div className="truncate">
                    {s.type === "main-category" || s.type === "sub-category" ? (
                      <>
                        <div className="font-medium text-gray-900 truncate flex items-center gap-1.5">
                          <i className="ri-folder-line text-blue-500 text-xs"></i>
                          {s.name}
                        </div>
                        {s.parentName && (
  ...
  return (
    <div
      className={`w-full relative z-20 flex items-center ${collapsible ? "w-full" : currentSize.container
        } ${className}`}
      ref={categoryDropdownRef}
    >
      <div
        className={`flex items-center bg-white border-2 border-blue-100 rounded-lg px-1 lg:px-3 py-0 lg:py-2 hover:border-blue-200 transition-all duration-300 w-full ${collapsible ? "animate-in slide-in-from-right-2 fade-in-0" : ""
          }`}
      >
        {showCategoryDropdown && (
          <div
          ...
          >
            ...
            <svg
              className={`w-3 h-3 text-primary transition-transform ${isCategoryDropdownOpen ? "rotate-180" : ""
                }`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
          ...
          >
            ...
            {suggestions.map((s, idx) => (
              <li
                ...
                onMouseEnter={() => setHighlightedIndex(idx)}
                className={`cursor-pointer px-3 py-2 text-sm flex justify-between items-center ${highlightedIndex === idx ? "bg-gray-100" : "hover:bg-gray-50"
                  }`}
              >
                <div className="truncate">
                  {s.type === "main-category" || s.type === "sub-category" ? (
