                    ...
                    >
                        {blogs?.map((blog, index) => (
                            <SwiperSlide key={index}>
                                <div onClick={() => navigate(`/blogs/${blog?._id}`)} className="group cursor-pointer border border-gray-300 rounded-2xl p-5 transition-all duration-300 hover:border-indigo-600">
                                    <div className="flex items-center mb-6">
                                        <img src={blog.image} alt="blog image" className="rounded-lg w-full object-cover" />
                                    </div>
                                    <div className="block">
                                        <h4 className="text-gray-900 font-medium leading-8 mb-3">{blog?.title}</h4>
                                        <div className="flex items-center justify-end font-medium">
                                            {/* <h6 className="text-sm text-gray-500">{blog?.author}</h6> */}
                                            <span className="text-sm text-indigo-600">Published At: {new Date(blog?.createdAt).toLocaleDateString(undefined, options)}</span>
                                        </div>
                                    </div>
                                </div>
                            </SwiperSlide>
