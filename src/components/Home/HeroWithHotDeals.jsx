...
const HeroWithHotDeals = () => {
  ...
  const { data: cmsData } = useCmsData("/api/general-cms/by-slug/hero-slider");
  const slider = Array.isArray(cmsData?.points) ? cmsData.points : [];
  ...
  return (
    <div className="mt-8">
      ...
      <div className="Mycontainer xl:h-[55vh]">
        <div className="flex flex-col xl:flex-row gap-6 xl:h-full">
          {/* Image Slider - 75% width */}
          <div className="w-full xl:w-3/4 xl:h-full min-h-[500px] sm:min-h-[450px] md:min-h-[400px] relative rounded-2xl overflow-hidden shadow-2xl">
            {/* Navigation Arrows */}
          ...
          <div className="w-full xl:w-1/4 xl:h-full">
            <HotDeals />
          </div>
        </div>
      </div>
