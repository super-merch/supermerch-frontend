...
const Sale = () => {
  ...
  const categories = [{ title: "TRENDINGS" }, { title: "NEW ARRIVALS" }, { title: "BEST SELLERS" }, { title: "SALE" }];

  // Distribute products among categories (ensuring different items in each)
  const chunkedProducts = categories.map((_, index) => {
          ...
          ? categories.map((_, index) => (
              <div key={index} className={`w-full mb-8 `}>
                ...
                <div className="space-y-4">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="flex items-center gap-3 p-4 border rounded-md animate-pulse">
          ...
          : categories.map((category, categoryIndex) => (
              <div key={categoryIndex} className={`w-full mb-8 ${categoryIndex % 2 === 0 ? "bg-gray-200" : "bg-gray-100"} rounded-md p-2 `}>
                ...
                <div className="flex items-center justify-between mb-4">
                  <Heading title={category.title} align="left" size="default" titleClassName="uppercase" containerClassName="" />
                  {chunkedProducts[categoryIndex].length > 0 && (
                    <button
                      onClick={() =>
                      ...
                      className="text-sm font-semibold border-b-2 text-smallHeader border-smallHeader"
                    >
                      VIEW ALL
                    </button>
                  )}
                          ...
                          >
                            ...
                            <div>
                              <p className="font-medium text-brand">
                                {product.overview.name || product.overview.name.length > 30
                                  ? product.overview.name.slice(0, 30) + "..."
                                  : "No Name "}
                              </p>
                              <p className="font-semibold text-smallHeader">${realPrice.toFixed(2)}</p>
                            </div>
                          </div>
                        );
                      })
