...
const ICON_MAP = { FaGlobe, FaUsers, FaBuilding, FaFlag, FaBox, FaProjectDiagram };

const MetricSec = () => {
  const { data: cmsData } = useCmsData("/api/general-cms/by-slug/about-metrics");
  const cmsMetrics = cmsData?.points?.metrics;

  const metrics = (Array.isArray(cmsMetrics) ? cmsMetrics : []).map((m) => ({
    ...m,
    icon: typeof m.icon === "string" ? (ICON_MAP[m.icon] || FaBuilding) : m.icon,
  }));
  ...
  return (
    ...
    >
      ...
      <div className="Mycontainer mx-auto px-4 relative z-10">
        ...
        {metrics.length === 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 lg:gap-8 mt-6 animate-pulse">
            <div className="h-28 rounded-2xl bg-white/20"></div>
            <div className="h-28 rounded-2xl bg-white/20"></div>
            <div className="h-28 rounded-2xl bg-white/20"></div>
            <div className="h-28 rounded-2xl bg-white/20"></div>
            <div className="h-28 rounded-2xl bg-white/20"></div>
