...
const ICON_MAP = { FaGlobe, FaHeart, FaUsers, FaLightbulb, FaEye, FaRocket };

const Version = () => {
  const { data: cmsData } = useCmsData("/api/general-cms/by-slug/about-vision");
  const cmsPillars = cmsData?.points?.visionPillars;

  const visionPillars = (Array.isArray(cmsPillars) ? cmsPillars : []).map((p) => ({
    ...p,
    icon: typeof p.icon === "string" ? (ICON_MAP[p.icon] || FaGlobe) : p.icon,
  }));
          ...
          <div className="space-y-8">
            ...
            {visionPillars.length === 0 ? (
              <div className="grid grid-cols-2 gap-4 animate-pulse">
                <div className="h-24 rounded-xl bg-white/20"></div>
                <div className="h-24 rounded-xl bg-white/20"></div>
                <div className="h-24 rounded-xl bg-white/20"></div>
                <div className="h-24 rounded-xl bg-white/20"></div>
              </div>
