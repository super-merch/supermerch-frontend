...
const ICON_MAP = { FaUsers, FaHeart, FaLightbulb, FaHandshake };

const Stable = () => {
  const { data: cmsData } = useCmsData("/api/general-cms/by-slug/about-inclusion");
  const cmsValues = cmsData?.points?.inclusionValues;

  const inclusionValues = (Array.isArray(cmsValues) ? cmsValues : []).map((v) => ({
    ...v,
    icon: typeof v.icon === "string" ? (ICON_MAP[v.icon] || FaUsers) : v.icon,
  }));
