...
const Checkout = () => {
  ...
  const handlePaymentSuccess = async (sessionId) => {
    try {
      ...
      // Clean up artwork preview keys from localStorage
      Object.keys(localStorage).forEach((key) => {
        if (key.startsWith("sm_artwork_")) localStorage.removeItem(key);
      });
      dispatch(clearCart());
    ...
    const checkoutData = {
      ...
      products: items.map((item) => {
        ...
        return {
          ...
          clearanceInfo: item?.clearanceInfo || null,
          adminCustomization: item.adminCustomization || null,
          addLogoLater: Boolean(item.addLogoLater),
