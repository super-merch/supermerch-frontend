...
const UserProducts = () => {
  ...
  const handleEditReorder = () => {
    ...
    if (!selectedOrder?.products?.length) {
      return toast.error("No products found in this order.");
    }
    ...
    selectedOrder.products.forEach((p) => {
      dispatch(
        addToCart({
          ...
          freightFee: p.freightFee || 0,
          size: p.size || "",
          print: p.print || "",
          printMethodKey: p.printMethodKey || p.print || "",
          logoColor: p.logoColor || "",
          logo: p.logo || "",
          marginFlat: p.marginFlat || 0,
    ...
    toast.success("Products added to cart. You can now edit and checkout.");
    navigate("/cart");
  };
    ...
    return (
      <div className="w-full px-3 lg:px-8 md:px-6 py-4 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            onClick={handleEditReorder}
            className="inline-flex items-center gap-2 px-4 py-2 mt-2 text-sm font-semibold text-white bg-secondary rounded-lg hover:bg-secondary/90 transition-colors"
          >
            Edit & Re-order
          </button>
        </div>
