...
const promotionalSlice = createSlice({
  ...
  reducers: {
    ...
    applyFilters: (state) => {
      const { filteredPromotionalProducts = [], selectedCategory, selectedBrands, searchText, minPrice, maxPrice } = state;
      ...
      const filtered = filteredPromotionalProducts.filter(
        (product) =>
          ...
          isBrandMatch(product) && isSearchMatch(product) && isPriceMatch(product)
      );
