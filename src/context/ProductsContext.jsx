...
const ProductsContextProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const fetchEffectivePricing = async () => {
            try {
                const payload = products
                    .filter((p) => p.meta?.id)
                    .map((p) => ({ productId: p.meta.id }));

                if (!payload.length) return;

                const { data } = await axios.post(
                    `${backendUrl}/api/add-discount/effective-bulk`,
                    { products: payload }
                );
                ...
                if (data.success && data.data) {
                    const discountMap = {};
                    const marginMap = {};
                    for (const [pid, vals] of Object.entries(data.data)) {
                        if (vals.discount) discountMap[pid] = vals.discount;
                        if (vals.margin) marginMap[pid] = vals.margin;
                    }
                    setTotalDiscount(discountMap);
                    setTotalMargin(marginMap);
                }
            } catch (error) {
